...
public class OpencorporaDictionaryParser {
  ...
  private void processG(String value) {
    ...
    if (Objects.nonNull(this.lemmaContext)) {
      this.lemmaContext.addProperty(grammeme);
    } else if (Objects.nonNull(this.formContext)) {
      this.formContext.addProperty(grammeme);
    }
  }
}
